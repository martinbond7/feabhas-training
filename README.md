...
**Note**: this Docker based training solution is under development
and designed as a potential alternative to Virtual Machines
...
Avoid placing the file in a folder that is mirrored using OneDrive, 
...
In the bottom left corner of the screen there will new be a green icon with 
an `><` symbol, click on this and select:

   * **Open Folder in container...** 

and open the folder you have justed created.

When VS Code opens the folder this will download a Docker container from 
`martinbond7/feabhas-target:latest`. This container is configured with a
...
The container is about 2.7GB and will take a noticeable amount 
of time to download.
...
a diagnostic interface on port 8888 which is maped for access from the host.
...
host operating system. A graphic representatyion of the emulated WMS 
hardware is provided by a Python script (`src/main.cpp`) that is run
on teh host and connects to the QEMU diagnostic port. 
...
click on the **Halt** button to stop the emulation. Closing the Python GUI 
will not stop the emulation; the Python GUI can connect to an already running
emulator.stop the QEMU simulation.
...
Preselect one of the laucnh options before pressing `<F5>` to debug with:
    ...
    * **QEMU debug container** to debug without the Python GUI
